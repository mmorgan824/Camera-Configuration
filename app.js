...
class CameraConfigApp {
    ...
    bindEvents() {
        ...
        // Also add click handler to trigger file input (redundant but safe)
        document.querySelector('.file-upload-btn')?.addEventListener('click', (e) => {
            // Prevent default to avoid double triggering
            e.preventDefault();
            // The file input is inside the label, so it will trigger automatically
